...
class CharacterBaseTest {
	...
	private static function runCharacterLifecycleCadence(smooth:Bool):{
		...
		var recoveryClock = new FrameClock(FrameRateSettings.fromQuery(query, true), new FrameRateDiagnostics(function():Float return 0));
		@:privateAccess FrameClock.setCurrentForTests(recoveryClock);
		var recovering = new Character();
		recovering.beginRecovery(8);
		while (@:privateAccess recovering.recoveryFrames > 0) {
			recoveryClock.advanceFrame();
			recovering.dispatchEvent(new Event(Event.ENTER_FRAME));
		}
		var recovery = '${recoveryClock.simulationFrameNumber}:${recoveryClock.stageFrameNumber}:${recovering.alpha}';
		recovering.remove();

		var invincibilityClock = new FrameClock(FrameRateSettings.fromQuery(query, true), new FrameRateDiagnostics(function():Float return 0));
		@:privateAccess FrameClock.setCurrentForTests(invincibilityClock);
		var invincible = new Character();
		invincible.becomeInvincible(135);
		while (@:privateAccess invincible.recoveryFrames > 0) {
			invincibilityClock.advanceFrame();
			invincible.dispatchEvent(new Event(Event.ENTER_FRAME));
		}
		var emitterKind = @:privateAccess invincible.activeParticleEmitter.kind;
		var invincibility = '${invincibilityClock.simulationFrameNumber}:${invincibilityClock.stageFrameNumber}:${invincible.alpha}:$emitterKind';
		invincible.remove();

		var fadeClock = new FrameClock(FrameRateSettings.fromQuery(query, true), new FrameRateDiagnostics(function():Float return 0));
		@:privateAccess FrameClock.setCurrentForTests(fadeClock);
		var fading = new Character();
		fading.beginRemove();
		while (!fading.removed) {
			fadeClock.advanceFrame();
			fading.dispatchEvent(new Event(Event.ENTER_FRAME));
		}
		var fade = '${fadeClock.simulationFrameNumber}:${fadeClock.stageFrameNumber}:${fading.removed}';
