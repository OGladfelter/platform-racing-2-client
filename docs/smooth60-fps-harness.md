...
`smooth60=1` mode it also requires every paired sample to satisfy the clock’s
exact alternating cadence: presented frames must equal twice the simulation
ticks, within one frame at a one-second window boundary. Without smooth mode,
